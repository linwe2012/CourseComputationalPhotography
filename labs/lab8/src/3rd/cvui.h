...
#ifdef CVUI_USE_CPP_FS
#include <filesystem>
#endif // CVUI_USE_CPP_FS
...
namespace render
{
	...
	void button(cvui_block_t& theBlock, int theState, cv::Rect& theShape, const cv::String& theLabel) {
		...
		if (theBlock.where.type() == CV_32FC1 || theBlock.where.type() == CV_32FC3) {
			scale = 255.0;
		}
		// Outline
		cv::rectangle(theBlock.where, theShape, cv::Scalar(0x29, 0x29, 0x29) / scale);

		// Border
		theShape.x++; theShape.y++; theShape.width -= 2; theShape.height -= 2;
		cv::rectangle(theBlock.where, theShape, cv::Scalar(0x4A, 0x4A, 0x4A) / scale);

		// Inside
		theShape.x++; theShape.y++; theShape.width -= 2; theShape.height -= 2;
		cv::rectangle(theBlock.where, theShape, theState == OUT ? 
			cv::Scalar(0x42, 0x42, 0x42) / scale : (theState == OVER ? cv::Scalar(0x52, 0x52, 0x52) / scale : cv::Scalar(0x32, 0x32, 0x32) / scale), CVUI_FILLED);
	}
...
public:
	...
	void stuff(const std::vector<Picture>& images, cv::Mat canvas, cv::Rect region, int row_cnt = 3, int col_cnt = 3) {
		using namespace cv;
		region_ = region;
		canvas_ = canvas;
		thumb_size_.width = region.width / col_cnt;
		thumb_size_.height = region.height / row_cnt;
		...
		for (auto& info : images) {
			...
			cv::resize(image, m, thumb_size_);
			if (canvas.type() == CV_32FC1 || canvas.type() == CV_32FC3) {
				m.convertTo(m, canvas.type(), 1 / 255.0);
			}
			Mat hover = m / 3;
			putText(hover, info.name.c_str(), { 10, thumb_size_.height - 30 }, HersheyFonts::FONT_HERSHEY_SIMPLEX, 0.5, { 255, 255, 255 });
			thumbs_.push_back({
				info.name,
				m,
				hover,
				ncols * thumb_size_.width,
				nrows * thumb_size_.height
				});
	...
	int pick() {
		...
		for (auto& thumb : thumbs_) {
			int aStatus = cvui::iarea(thumb.x, thumb.y, thumb_size_.width, thumb_size_.height);
			if (button(canvas_, region_.x + thumb.x, region_.y + thumb.y, thumb.thumb, thumb.hovered, thumb.hovered) ){
				clicked = cnt;
			}
			++cnt;
		}
...
#ifdef CVUI_USE_CPP_FS
	static std::vector<Picture> scan(std::string path) {
